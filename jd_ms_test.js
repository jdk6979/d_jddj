                ...
                if (vo['completionCnt'] < vo['assignmentTimesLimit']) {
                  if (vo['assignmentType'] === 1) {
                    ...
                    for (let i = vo['completionCnt']; i < vo['assignmentTimesLimit']; ++i) {
                      ...
                      // let shoppingActivity = JSON.parse(vo['ext']["shoppingActivity"])


                      let body = {
                        "encryptProjectId": $.encryptProjectId,
                        "encryptAssignmentId": vo['encryptAssignmentId'],
                        "itemId": vo['ext']["shoppingActivity"]?.[0]["itemId"],
                        // "itemId": shoppingActivity[0]["itemId"],
                        // "itemId": vo['ext'][vo['ext']['extraType']][i]['itemId'],

                      }

                      // https://happy.m.jd.com/babelDiy/zjyw/3ugedFa7yA6NhxLN5gw2L3PF9sQC/index.html?channel=248
                      
                      let url = vo['ext']["shoppingActivity"]?.[0]["url"];
                      // let url = shoppingActivity?.[0]["url"];
                      // let url = vo['ext']["shoppingActivity"][0];
...
function doTask(body,url) {
  ...
  return new Promise(resolve => {
    $.post(taskPostUrl_2('doInteractiveAssignment', body,url), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${err},${jsonParse(resp.body)['message']}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function taskPostUrl_2(function_id, body = {}, extra = '', function_id2,url) {
  // let url = `${JD_API_HOST}`;
  // let url = `${JD_API_HOST}`;
  ...
  url += `&componentId=6420bbdb74a747dca858ac24700a6619&taskParam=${escape(JSON.stringify(body))}&un_area=19_1607_4773_62121&lng=0&lat=0`;


  return {
    url,
    // body: `functionId=${function_id}&body=${escape(JSON.stringify(body))}&client=wh5&clientVersion=1.0.0&${extra}`,
    // body: `componentId=6420bbdb74a747dca858ac24700a6619&taskParam=${escape(JSON.stringify(body))}&un_area=19_1607_4773_62121&lng=0&lat=0`,
    // body: `&componentId=6420bbdb74a747dca858ac24700a6619&taskParam=${escape(JSON.stringify(body))}&un_area=19_1607_4773_62121&lng=0&lat=0`,
    // body: 'componentId=6420bbdb74a747dca858ac24700a6619&taskParam=${escape(JSON.stringify(body))}&un_area=19_1607_4773_62121&lng=0&lat=0',

    ///babelDiy/zjyw/3ugedFa7yA6NhxLN5gw2L3PF9sQC/index.html?channel=248&componentId=6420bbdb74a747dca858ac24700a6619&taskParam=%7B%22encryptProjectId%22%3A%224Rxa9S9Vcgmq5zVtPJzNdFwpzsar%22%2C%22encryptAssignmentId%22%3A%222Chw75t9MuDGiBek4rvY4mtwEXzL%22%2C%22itemId%22%3A%220801638966%22%7D&un_area=19_1607_4773_62121&lng=0&lat=0
...
// function taskPostUrl_2(function_id, body = {}, extra = '', function_id2) {
//   let url = `${JD_API_HOST}`;
//   if (function_id2) {
//     url += `?functionId=${function_id2}`;
//   }
